...
The top level IR is HLFIR. Though because MLIR allows mixing of dialects, it is
as if HLFIR and FIR sit aside one another.
```mlir
module attributes {<...>} {
...
```
The parameters are declared with `hlfir.declare` but their type is specified with `fir.array`. There is no need for HLFIR to have a `hlfir.array` if all it
would be doing is forwarding to `fir.array`.
...
At each of these stages the optimisations most suited to that level can
be done. For example, while we are in HLFIR we could optimise array accesses
because in HLFIR we have retained the information about how Fortran as a
language treats those.

If we were to try and do this later in LLVM IR it would be much more difficult.
Some optimisations correct for Fortran would in fact be impossible because
when given LLVM IR, the LLVM backend cannot make assumptions about the source
language. What is valid for Fortran may not be valid for another language.

# 

We're over 8 years since the first appearance of what has become LLVM Flang.

TODO: commit stats go here!

So it is time to hear from users, contributors and supporters past and present
about the impact Flang has had on their work.
