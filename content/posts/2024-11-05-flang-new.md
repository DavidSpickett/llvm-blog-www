...
If you want to try `flang` right now, you can download it or try it
in your browser using [Compiler Explorer](https://godbolt.org/z/3hhYM37Kh).

TODO: llvm release link

# Why Fortran?

This article does not require any Fortran knowledge, but I will describe the
context Fortran was created in and why it continues to thrive.
...
Primer you will see why. Fortran focused on the mathematical use case and freed
programmers from writing machine specific assembly code.

Instead they could write code that looked like a formula. Something we
...
If you want to learn more about Fortran, [fortran-lang.org](https://fortran-lang.org/)
is a great place to start.

# Why Make Another Fortran Compiler?
...
Why build yet another compiler?

Let us look at the motivations of the US National Labs and NVIDIA. The two partners
in the early days of Flang.

For Pat McCormick (Flang project lead at Los Alamos National Laboratory) preserving
the utility of new and existing Fortran code was imperative:
...
is a single point of failures, incompatibilities and disagreements. Multiple
implementations reduces that risk, and as we see with GCC and Clang, pushes forward
...
assumptions that may be a problem for one compiler or another, or tradeoffs that
affect certain types of compiler user differently.
...
Something that Jeff Hammond (NVIDIA) has spent a lot of time doing:
...
history of the Portland Group (particularly their engagement with US National Labs)
...
* **May 2017** The first source code release of Classic Flang as a separate
  ...
  and a new backend that generates LLVM IR.
* **August 2017** The Classic Flang project is [announced officially](https://llvmweekly.org/issue/191) (LLVM Weekly's report, the original mailing list is offline).
...
Confusion about the `flang` name was a key motivation for Brad:
...
* Many performance and correctness issues were addressed by the "High Level Fortran Intermediate Representation" (HLFIR).
...
on [Compiler Explorer](https://godbolt.org/z/3hhYM37Kh). You know at least one person will appreciate it!
...
languages (Fortran array optimisations were cited as a use case for
MLIR).
...
> Fortran remains under active development as a standard, supporting both legacy
> and modern codebases. The need for a more modern Fortran compiler motivated the
> creation of the LLVM Flang project and AMD fully supports that path.
>
> In following with community trends, AMD’s Next-Gen Fortran Compiler will be
...
> to directly leverage the power of AMD’s GPUs. AMD’s Next-Gen Fortran Compiler’s
> goal is fulfilling our vision by allowing you to deploy and accelerate your Fortran
> codes on AMD GPUs using OpenMP offloading, and to directly interface and invoke HIP
> and ROCm kernels.
